...
当前核心基于官方 v2.6.7，应用了
`patches/subs-check-pro-v2.6.7-custom-rename.patch`。开启 IP 归属地重命名后，
节点使用 `🇭🇰 香港 01 | 1x` 格式：地区内编号补齐为两位，原名存在倍率时
保留并统一为小写 `x`；原名没有倍率时不虚构倍率。IP 归属查询失败时保留原名。

为了防止官方自动更新覆盖本地补丁，核心自动更新已关闭。升级核心前需要把补丁迁移到
目标版本、重新运行测试并构建二进制。
